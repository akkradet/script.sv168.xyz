...
if [ ! -f $public_file ];then
	wget -O $public_file http://script.sv168.xyz/install/public.sh -T 5;
fi
. $public_file
...
Install_deployment()
{
	...
	if [ "$?" -ne 0 ];then
		...
		wget -O /www/server/panel/plugin/deployment/deployment_main.py $download_Url/install/plugin/deployment_en/deployment_main.py -T 5
		wget -O /www/server/panel/plugin/deployment/index.html $download_Url/install/plugin/deployment_en/index.html -T 5
		wget -O /www/server/panel/plugin/deployment/info.json $download_Url/install/plugin/deployment_en/info.json -T 5
		wget -O /www/server/panel/plugin/deployment/list.json $download_Url/install/plugin/deployment_en/list.json -T 5
		wget -O /www/server/panel/plugin/deployment/type.json $download_Url/install/plugin/deployment_en/type.json -T 5
		wget -O /www/server/panel/plugin/deployment/icon.png $download_Url/install/plugin/deployment_en/icon.png -T 5
		wget -O /www/server/panel/static/img/soft_ico/ico-deployment.png $download_Url/install/plugin/deployment_en/icon.png -T 5
	fi
